...
class GameController:
    def __init__(self, window_name):
        ...
        self.bank_6_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\bank_6.png"
        self.bank_7_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\bank_7.png"
        self.bank_8_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\bank_8.png"

        self.town_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\town.png"
    ...
    def log_upgrade_attempt(self, from_level, to_level, result):
        """
        Log individual upgrade attempt
        
        Parameters:
        - from_level: Starting level (1, 2, 3, 4, 5, 6)  
        - to_level: Target level (2, 3, 4, 5, 6, 7)
        - result: 'success', 'failed', 'cannot_perform'
        """
        # Always track total upgrades
        self.upgrade_stats['total_upgrades'] += 1
        
        # Print upgrade attempt and result
        print(f"📊 UPGRADE #{self.upgrade_stats['total_upgrades']}: +{from_level}→+{to_level} = {result.upper()}")
        
        # Calculate cost based on result and item level
        if result == 'success':
            # Success: Only the scroll used (10k)
            cost = 10000
        elif result == 'failed':
            # Failed (item burned): All scrolls invested in that item
            # +1 burns = 10k (1 scroll), +2 burns = 20k (2 scrolls), +3 burns = 30k (3 scrolls), +4 burns = 40k (4 scrolls), +5 burns = 50k (5 scrolls), +6 burns = 60k (6 scrolls)
            cost = 10000  #(from_level - 1) * 10000
        else:
            # Cannot perform or unclear: Just the scroll attempted
            cost = 10000
        
        self.upgrade_stats['total_cost'] += cost
        
        # Only track success/failure statistics for risky upgrades (+3→+4, +4→+5, +5→+6, +6→+7)
        upgrade_key = None
        if from_level == 3 and to_level == 4:
            upgrade_key = '3_to_4'
        elif from_level == 4 and to_level == 5:
            upgrade_key = '4_to_5'
        elif from_level == 5 and to_level == 6:
            upgrade_key = '5_to_6'
        elif from_level == 6 and to_level == 7:
            upgrade_key = '6_to_7'
        
        # Only log probability data for risky upgrades
        if upgrade_key:
            self.upgrade_stats['upgrade_attempts'][upgrade_key]['attempts'] += 1
            
            if result == 'success':
                self.upgrade_stats['upgrade_attempts'][upgrade_key]['successes'] += 1
                if to_level == 7:  # Count +7 achievements
                    self.upgrade_stats['plus_seven_achieved'] += 1
            elif result == 'failed':
                self.upgrade_stats['upgrade_attempts'][upgrade_key]['failures'] += 1
        else:
            # For +1→+2 and +2→+3, still count +7 achievements if somehow they reach +7
            if to_level == 7 and result == 'success':
                self.upgrade_stats['plus_seven_achieved'] += 1
    
    def calculate_upgrade_probabilities(self):
        """Calculate success probabilities for each upgrade level"""
        probabilities = {}
        
        for level, stats in self.upgrade_stats['upgrade_attempts'].items():
            if stats['attempts'] > 0:
    ...
    def save_upgrade_statistics(self, filename=None):
        """Save upgrade statistics to text file"""
        import os
        
        # Create game_states folder if it doesn't exist
        folder_name = "game_states"
        ...
        if filename is None:
            ...
            filename = f"{folder_name}/upgrade_stats_{timestamp}.txt"
        
        probabilities = self.calculate_upgrade_probabilities()
        
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("=" * 60 + "\n")
            f.write("KNIGHT ONLINE UPGRADE STATISTICS\n")
            f.write("=" * 60 + "\n")
            f.write(f"Session Start: {self.upgrade_stats['session_start']}\n")
            f.write(f"Session End: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Total Upgrades: {self.upgrade_stats['total_upgrades']}\n")
            f.write(f"Total Cost: {self.upgrade_stats['total_cost']:,} coins\n")
            f.write(f"+7 Items Achieved: {self.upgrade_stats['plus_seven_achieved']}\n")
            f.write("\n" + "=" * 60 + "\n")
            f.write("UPGRADE PROBABILITIES (Risky Upgrades Only)\n")
            f.write("=" * 60 + "\n")
            f.write("Note: +1→+2 and +2→+3 have 100% success rate (not tracked)\n")
            f.write("Only tracking statistics for upgrades that can fail:\n\n")
            
            for level, prob in probabilities.items():
                level_name = level.replace('_', ' → +')
                f.write(f"+{level_name} Upgrades:\n")
                f.write(f"  Attempts: {prob['attempts']}\n")
                f.write(f"  Successes: {prob['successes']}\n")
                f.write(f"  Failures: {prob['failures']}\n")
                f.write(f"  Success Rate: {prob['success_rate']}%\n\n")
            
            f.write("\n" + "=" * 60 + "\n")
            f.write("COST ANALYSIS\n")
            f.write("=" * 60 + "\n")
            #f.write(f"Cost per upgrade attempt: 10,000 coins\n")
            f.write(f"Total attempts: {self.upgrade_stats['total_upgrades']}\n")
            f.write(f"Total spent: {self.upgrade_stats['total_cost']:,} coins\n")
            if self.upgrade_stats['plus_seven_achieved'] > 0:
                cost_per_plus_seven = self.upgrade_stats['total_cost'] / self.upgrade_stats['plus_seven_achieved']
                f.write(f"Average cost per +7 item: {cost_per_plus_seven:,.0f} coins\n")
        
        print(f"📊 Upgrade statistics saved to: {filename}")
        return filename
    
    def print_current_statistics(self):
        """Print current upgrade statistics to console"""
        probabilities = self.calculate_upgrade_probabilities()
        
        print("\n" + "=" * 50)
        print("📊 CURRENT UPGRADE STATISTICS")
        print("=" * 50)
        print(f"Total Upgrades: {self.upgrade_stats['total_upgrades']} (includes +1→+2, +2→+3)")
        print(f"Total Cost: {self.upgrade_stats['total_cost']:,} coins")
        print(f"+7 Items Achieved: {self.upgrade_stats['plus_seven_achieved']}")
        print("\n🎯 Risky Upgrade Success Rates:")
        
        for level, prob in probabilities.items():
            if prob['attempts'] > 0:
                level_name = level.replace('_', ' → +')
                print(f"  +{level_name}: {prob['success_rate']}% ({prob['successes']}/{prob['attempts']})")
        
        if not any(prob['attempts'] > 0 for prob in probabilities.values()):
            print("  No risky upgrades attempted yet")
        print("=" * 50)
        ...
        self.hesta_7_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\hesta_7.png"
        self.hesta_8_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\hesta_8.png"
        self.hesta_9_path = r"C:\Users\ismt\Desktop\python-ann\knightonlinefarmagentproject\ko_upgrade\game_images\hesta_9.png"
    ...
    def perform_single_upgrade_with_template_detection(self, anvil_rects, locked_slots, item_slot_to_upgrade, expected_initial_level, inventory_data=None):
        ...
        try:
            # Read initial item level to confirm
    ...
    def open_npc(self, npc="anvil"):
        ...
        center_x = 512
        center_y = 300 if npc == "anvil" else 390
    ...
    def town_(self):
        ...
        for attempt in range(max_attempts):
            ...
            self.click_(1024//2, 768//2, clicks=2)
            time.sleep(1)
            #buraya emin olmak için bir template eklenebilir...
            location = self.find_template(self.confirm_path, confidence=0.7)
            if location:
                time.sleep(0.5)
                self.click_(location[0], location[1])
            found = False        
            while not found:
                self.press_('h', hold_duration=0.1)
                time.sleep(1)
                location = self.find_template(self.town_path, confidence=0.7)
                if location:
                    found = True                
    ...
    def process_inventory_with_model(self, model):
        """
        Process current inventory with ML model (assumes inventory is already open)
        ...
        - dictionary: Complete inventory data with ML predictions and template values
        """
        # Get inventory rectangles and locked slots (assuming they're available as instance variables)
        if not hasattr(self, 'inventory_rects') or not hasattr(self, 'locked_slots'):
            print("❌ Missing inventory_rects or locked_slots - using defaults")
            # Use default values if not set
            inventory_rects = [(44, 105), (163, 122)]  # Default inventory area
            locked_slots = set()  # No locked slots by default
        else:
            inventory_rects = self.inventory_rects
            locked_slots = self.locked_slots
        
        # Capture screenshot and get ML predictions
        print("🧠 Getting ML predictions for current inventory...")
        screenshot = self.capture_game_window()
        ...
        # Build inventory data using existing method
        inventory_data = self.rebuild_inventory_data_after_restock(predictions, inventory_rects, locked_slots)
        
        if inventory_data:
            print("✅ Inventory processing completed successfully!")
        else:
            print("❌ Failed to build inventory data")
    ...
    def find_correct_npc_at_armor(self):
        ...
        if gargamet_found:
            ...
            self.press_("d", hold_duration=1.75)
            time.sleep(0.5)
    ...
    def interact_with_bank(self):
        ...
        bank_paths = {
            ...
            "bank_6": self.bank_6_path,
            "bank_7": self.bank_7_path,
            "bank_8": self.bank_8_path,
            ...
            if anvil_success:
                ...
                if attempt < max_attempts - 1:  # Don't go to town on last attempt
                    ...
                    if town_success:
                        ...
                        anvil_nav_success = nav_system.navigate_with_agent("town_to_anvil", max_steps=75, distance_threshold=5)
    ...
    def calculate_items_to_buy(self, item_counts, empty_slots):
        ...
        for i, item in enumerate(purchase_order):
            ...
            print(f"   {i+1:2d}. armor npc slot {item['sundires_slot']}: {item_name} (was {item['current_count']})")
        ...
        for i, item_info in enumerate(purchase_order):
            try:
                ...
                if sundires_slot >= len(sundires_rects):
                    print(f"❌ Invalid armor slot {sundires_slot} (max: {len(sundires_rects)-1})")
                    continue
                ...
                print(f"\n🎯 Purchase {i+1}/{len(purchase_order)}: Buying {item_name} from armor npc slot {sundires_slot}...")
    ...
    def upgrade_all_items_to_target_level(self, anvil_rects, inventory_data, locked_slots, target_from_level=1, target_to_level=3):
        ...
        while current_level < target_to_level:
            ...
            for item_info in items_to_upgrade:
                ...
                # Log upgrade attempt for statistics
                self.log_upgrade_attempt(current_level, current_level + 1, result)
                
                # Simple upgrade result logging
                result_icon = "✅" if result == "success" else "💥" if result == "failed" else "🚫"
                new_level = current_level + 1 if result == "success" else "BURNED"
                print(f"Slot {slot_idx}: {item_name} +{current_level} → {new_level} {result_icon}")
                ...
                if result == "success":
                    ...
                    stats_file = self.save_upgrade_statistics()
    ...
    def navigate_to_bank_and_store_plus_seven_items(self, nav_system, inventory_data, locked_slots):
        ...
        anvil_to_armor_success = nav_system.navigate_with_agent("anvil_to_armor", max_steps=125, distance_threshold=4)
        ...
        if anvil_to_armor_success:
            ...
            armor_to_inn_success = nav_system.navigate_with_agent("armor_to_inn", max_steps=125, distance_threshold=4)
        ...
        if not bank_nav_success:
            ...
            town_to_anvil_success = nav_system.navigate_with_agent("town_to_anvil", max_steps=75, distance_threshold=12)
            if not town_to_anvil_success:
                print("❌ Failed to reach anvil from town")
                return False, inventory_data
            
            print("🔨➡️🛡️ Going from anvil to armor...")
            anvil_to_armor_success = nav_system.navigate_with_agent("anvil_to_armor", max_steps=100, distance_threshold=4)
        ...
        if not bank_interaction_success:
            ...
            print("🏠➡️🔨 Going from town to anvil...")
            town_to_anvil_success = nav_system.navigate_with_agent("town_to_anvil", max_steps=100, distance_threshold=12)
            if not town_to_anvil_success:
                print("❌ Failed to reach anvil from town")
                return False, inventory_data
    ...
    def execute_restock_cycle(self, nav_system, inventory_data, locked_slots, empty_count):
        ...
        try:
            ...
            anvil_nav_success = nav_system.navigate_with_agent("town_to_anvil", max_steps=75, distance_threshold=12)
            ...
            armor_nav_success = nav_system.navigate_with_agent("anvil_to_armor", max_steps=100, distance_threshold=4)
    ...
    def advanced_upgrade_system_to_plus_seven(self, anvil_rects, inventory_data, locked_slots, nav_system, model=None):
        ...
        while cycle_count < max_cycles:
            ...
            if success_achieved:
                ...
                if bank_success:
                    ...
                    # Display current statistics after banking +7 items
                    self.print_current_statistics()
            ...
            # Priority 6: Upgrade +6 items to +7 (final goal)
                ...
                if batch_size > 0:
                    ...
                    # Check empty slots before deciding
                    empty_slots = self.count_empty_slots(inventory_data)
                    
                    if empty_slots >= 10:
                        print("🏠 Going to town to make decision (empty slots >= 10)...")
                        town_success = self.town_()
                        ...
                        print(f"🔥 Empty slots ({empty_slots}) < 10 - continuing with individual +6→+7 upgrades...")
                    
                    # Continue with individual upgrades (either failed to reach town or empty slots < 10)
                    inventory_data, upgrade_result = self.upgrade_all_items_to_target_level(
                        anvil_rects, inventory_data, locked_slots,
                        target_from_level=6, target_to_level=7
                    )
        ...
        # Save and display final statistics
        self.print_current_statistics()
        stats_file = self.save_upgrade_statistics()
    ...
    def execute_bank_storage_with_retry(self, nav_system, inventory_data, locked_slots, max_attempts=10):
        """
    ...
    def execute_restock_with_retry(self, nav_system, inventory_data, locked_slots, empty_count, max_attempts=10):
        """
...
def startup_town_based_system(controller, nav_system, model, locked_slots):
    ...
    for attempt in range(max_startup_attempts):
        ...
        if level_counts[7] >= 6:
            ...
            bank_success = controller.execute_bank_storage_with_retry(nav_system, inventory_data, locked_slots, max_attempts=10)
            if bank_success:
            ...
            restock_success, purchased_items = controller.execute_restock_with_retry(nav_system, inventory_data, locked_slots, empty_count, max_attempts=10)
            if restock_success:
            ...
            anvil_success = nav_system.navigate_with_agent("town_to_anvil", max_steps=75, distance_threshold=5)
            
            if anvil_success:
                print("✅ Successfully reached anvil!")
                
                # Interact with anvil
                anvil_interaction_success = controller.execute_anvil_interaction_with_retry(nav_system, max_attempts=10)
                ...
                if anvil_interaction_success:
                    ...
                    upgrade_result = controller.advanced_upgrade_system_to_plus_seven(anvil_rects, inventory_data, locked_slots, nav_system, model)
                    
                    # Handle upgrade result
                    if isinstance(upgrade_result, tuple) and len(upgrade_result) == 3:
                        updated_inventory, session_stats, success = upgrade_result
                        
                        if success:
                            print("🎉 UPGRADE SYSTEM ACHIEVED GOAL!")
                            return upgrade_result
                        else:
                            print("🔄 UPGRADE SYSTEM RETURNED TO TOWN FOR DECISION")
                            print("🏠 Restarting town-based decision system...")
                            continue  # Restart the decision loop
                    else:
                        print("⚠️ Unexpected result from upgrade system")
                        return upgrade_result
                else:
    ...
    print("🏠 FALLBACK: Forcing town navigation and continuing with basic decision system...")
    
    # Fallback: Just go to town and continue with decision loop
    try:
        controller.town_()
        print("✅ Fallback town navigation successful - continuing with decision system")
    except Exception as e:
        print(f"❌ Even fallback town failed: {e}")
        print("🔄 RESILIENT MODE: Continuing anyway - decision loop will handle this")
    
    # START CONTINUOUS DECISION LOOP
    print("\n🔄 STARTING CONTINUOUS DECISION LOOP...")
    print("🎯 Flow: Town → Open Inventory → Analyze → Route to Action → Repeat")
    print("=" * 70)
    
    decision_loop_count = 0
    max_continuous_loops = 1000  # Safety limit to prevent infinite loops
    
    while decision_loop_count < max_continuous_loops:
        try:
            decision_loop_count += 1
            print(f"\n🔄 DECISION LOOP #{decision_loop_count}")
            print("🏠 Ensuring we're at town...")
            
            # Step 1: Ensure at town
            town_success = controller.town_()
            if not town_success:
                print("❌ Failed to reach town, retrying...")
                continue
            
            # Step 2: Open inventory and scan
            print("🎒 Opening inventory to check current state...")
            inventory_success = controller.open_inventory()
            if not inventory_success:
                print("❌ Failed to open inventory, retrying...")
                continue
            
            # Get fresh inventory scan
            print("🔍 Scanning inventory...")
            try:
                inventory_data = controller.process_inventory_with_model(model)
                if not inventory_data:
                    print("❌ Failed to scan inventory, retrying...")
                    continue
            except Exception as e:
                print(f"❌ Inventory scan error: {e}, retrying...")
                continue
            ...
            print(f"   +7 items: {plus_seven_count}")
            print(f"   Empty slots: {empty_count}")
            print(f"   Total levels: {level_counts}")
            
            # Close inventory before decision
            controller.close_inventory()
            
            # Step 4: Make routing decision
            if plus_seven_count >= 10:
                print("🎉 MISSION ACCOMPLISHED! 10+ +7 items achieved!")
                return inventory_data, {'plus_seven_achieved': plus_seven_count}, True
            
            elif plus_seven_count >= 5:
                print(f"🏦 Banking {plus_seven_count} +7 items for safekeeping...")
                bank_success, updated_inventory = controller.store_plus_seven_items_in_bank(inventory_data, locked_slots, nav_system)
                if bank_success:
                    print("✅ Banking completed, restarting from town...")
                    inventory_data = updated_inventory
                    continue
                else:
                    print("⚠️ Banking failed, continuing with upgrades...")
            
            elif empty_count > 10:
                print(f"🛒 Need restocking: {empty_count} empty slots > 10")
                print("🔄 Going to armor shop to buy items...")
                restock_success = controller.perform_restocking_cycle(nav_system, max_attempts=3)
                if restock_success:
                    print("✅ Restocking completed, restarting from town...")
                    continue
                else:
                    print("⚠️ Restocking failed, trying upgrades anyway...")
            
            # Step 5: Default action - Go to anvil for upgrades
            print("🔨 Going to anvil for upgrade operations...")
            
            anvil_success = nav_system.navigate_with_agent("town_to_anvil")
            if not anvil_success:
                print("❌ Failed to reach anvil, retrying from town...")
                continue
            
            anvil_open_success = controller.open_npc("anvil")
            if not anvil_open_success:
                print("❌ Failed to open anvil, retrying from town...")
                continue
            
            # Anvil upgrade operations
            print("🚀 Starting upgrade operations at anvil...")
            anvil_top_left = (581, 334)
            anvil_bottom_right = (695, 481)
            anvil_rects = controller.draw_rectangle_grid(None, anvil_top_left, anvil_bottom_right, rows=4, cols=7, spacing=7)
            ...
            if isinstance(upgrade_result, tuple) and len(upgrade_result) == 3:
                updated_inventory, session_stats, success_achieved = upgrade_result
                
                if success_achieved:
                    print("🎉 UPGRADE SYSTEM ACHIEVED GOAL!")
                    return upgrade_result
                else:
                    print("🔄 Upgrade cycle completed, restarting decision loop...")
                    continue
            else:
                print("⚠️ Unexpected result from upgrade system, restarting...")
                continue
                
        except Exception as e:
            print(f"❌ Error in decision loop {decision_loop_count}: {e}")
            print("🔄 Continuing to next iteration...")
            continue
    
    print(f"⚠️ Reached maximum decision loops ({max_continuous_loops}), ending session")
    return False
...
def main():
    try:
        ...
        if result:
            ...
            print("❌ TOWN-BASED SYSTEM ENCOUNTERED ISSUES!")
            print("🔄 RESILIENT MODE: Going to town and restarting decision system...")
            
            # Instead of failing, go to town and restart
            try:
                controller.town_()
                print("✅ Recovery successful - restarting decision system...")
                # Restart the whole decision system
                startup_success = startup_town_based_system(controller, nav_system, model, locked_slots)
                if startup_success:
                    print("✅ RECOVERY SUCCESSFUL - SYSTEM RESTARTED!")
                else:
                    print("🔄 CONTINUOUS MODE: System will keep trying...")
            except Exception as e:
                print(f"❌ Recovery attempt failed: {e}")
                print("🔄 PERSISTENT MODE: System continues despite errors...")
