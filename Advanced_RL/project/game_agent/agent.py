...
from network import create_deep_q_network
...
class Agent:
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims,
                 ...
                 replace=250, algo=None, env_name=None, chkpt_dir='tmp/dqn'):
        self.gamma = gamma
        ...
        self.q_eval = create_deep_q_network(input_dims, n_actions)
        self.q_eval.compile(optimizer=Adam(learning_rate=lr))
        self.q_next = create_deep_q_network(input_dims, n_actions)
    ...
    def learn(self):
        ...
        with tf.GradientTape() as tape:
            ...
            # max_actions.shape = (32,)        -> The index of the action with the highest Q-value for each next state
            max_actions = tf.math.argmax(q_next, axis=1, output_type=tf.int32)
            # max_action_idx.shape = (32, 2)   -> Stack of indices and max_actions, used to gather max Q-values
            max_action_idx = tf.stack([indices, max_actions], axis=1)
            # q_target.shape = (32,)           -> Target Q-value for each state-action pair in the batch
            # This is the Bellman target: reward + gamma * max(Q_next)
            q_target = rewards + \
                self.gamma*tf.gather_nd(q_next, indices=max_action_idx) *\
                (1 - dones.numpy()) # (1 - dones) makes sure we don't include future rewards if the episode ended
